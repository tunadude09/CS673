...
## demo
gtm.demo <-function (randomSetup = FALSE, points = 5, samples = 9,
                     interactive=TRUE) {
  ##TT = matrix(3:61/20, ncol = 1)
  ##TT = cbind(TT, TT + 1.25 * sin(2 * TT))
  
  TT = matrix(1:100, ncol=10)
  plot_begin = 0
  plot_end = 30
  
  
  if(interactive) {
    plot(TT[, 1], TT[, 2], col = "red", pch = 21, xlim = c(plot_begin, plot_end), 
         ylim = c(plot_begin, plot_end))
  ...
  for (j in 1:15) {
    ...
    Y = FI %*% W
    if(interactive) {
      plot(TT[, 1], TT[, 2], col = "red", xlim = c(plot_begin, plot_end), ylim = c(plot_begin, plot_end))
